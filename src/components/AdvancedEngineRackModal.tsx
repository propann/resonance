...
export const AdvancedEngineRackModal: React.FC<Props> = ({ isOpen, onClose, libraryRoot }) => {
  ...
  return (
    <Modal
      ...
      title="Rack Extensions · 10 moteurs"
      subtitle="Moteurs séparés du rack principal · activation individuelle à la demande"
      headerRight={
        <button
          onClick={resetAll}
          className="shrink-0 rounded border border-[#55451D] px-2 py-1 text-[10px] text-[#FFE08A]"
        >
          RESET
        </button>
      }
    >
      <div className="grid gap-3 sm:grid-cols-2">
        {ENGINES.map((engine) => {
          ...
          const values = params[engine.id] || defaultParams();
          const updateParam = (key: keyof EngineParams, value: number) =>
            setParams((current) => ({ ...current, [engine.id]: { ...values, [key]: value } }));
          return (
            <article
              key={engine.id}
              className={`rounded-lg border p-4 ${isActive ? 'border-[#00F0FF] bg-[#00F0FF]/5' : 'border-[#2A2934] bg-[#11121A]'}`}
            >
              <div className="flex justify-between gap-2">
                <div>
                  <h3 className="text-sm font-bold">{engine.name}</h3>
                  <div className="mt-1 text-[10px] text-[#00F0FF]">{engine.family}</div>
                </div>
                <span className={`text-[9px] font-bold ${isActive ? 'text-[#00F0FF]' : 'text-[#77778A]'}`}>
                  {isActive ? 'ACTIF' : 'INACTIF'}
                </span>
              </div>
              <p className="mt-3 min-h-8 text-[10px] text-[#A9A9B8]">{engine.description}</p>
              <div className="mt-3 space-y-1.5">
                {([['mix', 'Mix'], ['tone', 'Tone'], ['morph', 'Morph']] as const).map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2 text-[10px] text-[#A9A9B8]">
                    ...
                    <input
                      ...
                      className="flex-1 accent-[#FFB000]"
                    />
                    <span className="w-7 text-right">{Math.round(values[key] * 100)}</span>
                  </label>
                ))}
              </div>
              <button
                ...
                className={`mt-4 w-full rounded py-2 text-[10px] font-bold disabled:opacity-50 ${isActive ? 'bg-[#252737] text-[#EDEDEE]' : 'bg-[#FFB000] text-black'}`}
              >
                {loading === engine.id ? (
                  <>
                    <Download className="mr-1 inline h-3 w-3 animate-bounce" />
                    Chargement…
                  </>
                ) : isActive ? (
                  'DÉSACTIVER'
      ...
      <p className="mt-4 text-[10px] text-[#77778A]">
        Les extensions sont mémorisées dans les préférences. Leur pont WASM/audio natif sera branché
        sans alourdir le démarrage principal.
      </p>
    </Modal>
  );
};
